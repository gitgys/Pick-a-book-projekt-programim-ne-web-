  ...
  if($type == "searchBox"){
    for($i = 0; $i < 5; $i++){
      ...
      if($rowForEmer == NULL){
        ...
        echo "<div id = 'perdorues'> <a href='http://localhost/PW/AdminPage/librariaPerdoruesit.php?perdoruesId=".$rowForEmer['perdoruesId']."'>".$rowForEmer['emri']."</a></div>"?><br><?php
      }
    }
    ...
    while(true){
      ...
      if($rowForEmer = mysqli_fetch_array($resultForEmer, MYSQLI_BOTH)){
        ...
        echo "</td>";
        echo "<td>";
        echo "<a href='http://localhost/PW/AdminPage/librariaPerdoruesit.php?perdoruesId=".$rowForEmer['perdoruesId']."'><button class='btn btn-success'>Perditeso ".$rowForEmer['emri']."</button></a>";
        echo "</td>";
        echo "</tr>";
